...
import com.chaquo.python.console.MainActivity;

import java.util.ArrayList;
import java.util.Set;

public abstract class BacNetActivity extends AppCompatActivity
    ...
    //private enum instanceType {MYFEED, DEBUG};

    private static ArrayList<BacNetActivity> openActivities = new ArrayList<BacNetActivity>();
    ...
    public BacNetActivity(){
        super();
        Log.d("BACNET_TAG", this.toString());
        //if (!instanced){
        //    openActivities = new ArrayList<BacNetActivity>();
        //}
        boolean found = false;
        for (BacNetActivity activity: openActivities){
            if(this.getClass().equals(activity.getClass())){
                found = true;
                break;
            }
        }
        if (found){
            openActivities.add(this);
        }
    }
    ...
    /*
    @Override public boolean onOptionsItemSelected(MenuItem item) {
        Log.d("ACTIVITY", "" + openActivities.size());
        int id = item.getItemId();
        if (id == resId("id", "menu_myFeed")) {
            if (!(this instanceof MyFeedActivity)) {
                //boolean found = false;

                for(BacNetActivity activity : openActivities){
                    if (activity.getClass().equals(MyFeedActivity.class)){
                        switchTo(activity);
                        found = true;
                        break;
                    }
                ...
                switchTo(MyFeedActivity);
                    openMyFeedActivity();
            }

        }

        else if (id == resId("id", "menu_Debug")){
...
*/
    private void switchTo(BacNetActivity activity) {
        Intent openActivity = new Intent(this, activity.getClass());
        openActivity.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
        startActivityIfNeeded(openActivity, 0);
        /*
    ...
    protected void onCreate(Bundle savedInstanceState) {
        Log.d("create_tag", this.toString());
    ...
    public void openMyFeedActivity(){
        Intent openActivity = new Intent(this, MyFeedActivity.class);
        openActivity.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
        startActivityIfNeeded(openActivity, 0);
        /*
